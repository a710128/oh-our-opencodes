...
The `src/cli/` directory provides the command-line interface for installing and configuring **oh-our-opencodes**, an OpenCode plugin. It handles:
...
```
User runs: bunx oh-our-opencodes install
         │
         ▼
┌─────────────────────────────────────────┐
│ index.ts: parseArgs()                   │
...
| `oh-our-opencodes.json` | `~/.config/opencode/` | Plugin-specific config |

### Consumers

1. **End Users**: Via `bunx oh-our-opencodes install`
