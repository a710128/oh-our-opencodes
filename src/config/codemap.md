...
- User config: `~/.config/opencode/oh-our-opencodes.json` (or `$XDG_CONFIG_HOME`)
- Project config: `<directory>/.opencode/oh-our-opencodes.json`
- Environment override: `OH_OUR_OPENCODES_PRESET`
...
│
├─→ Load user config from ~/.config/opencode/oh-our-opencodes.json
│   └─→ Validate with PluginConfigSchema
│       └─→ Return null if invalid/missing
│
├─→ Load project config from <directory>/.opencode/oh-our-opencodes.json
│   └─→ Validate with PluginConfigSchema
│       └─→ Return null if invalid/missing
│
...
│
├─→ Apply environment preset override (OH_OUR_OPENCODES_PRESET)
│
...
│
├─→ Check ~/.config/opencode/oh-our-opencodes/{agentName}.md
│   └─→ If exists → read as replacement prompt
│
└─→ Check ~/.config/opencode/oh-our-opencodes/{agentName}_append.md
