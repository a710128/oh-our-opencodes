...
Complete guide for using tmux integration with oh-our-opencodes to watch agents work in real-time through automatic pane spawning.
...
Edit `~/.config/opencode/oh-our-opencodes.json` (or `.jsonc`):
...
Configure tmux behavior in `~/.config/opencode/oh-our-opencodes.json` (or `.jsonc`):
...
1. **Verify tmux integration is enabled:**
   ```bash
    cat ~/.config/opencode/oh-our-opencodes.json | grep tmux # (or .jsonc)
   ```
